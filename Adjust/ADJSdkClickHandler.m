...
- (void)sendNextSdkClickI:(ADJSdkClickHandler *)selfI
{
    ...
    ADJActivityPackage *sdkClickPackage = [selfI.packageQueue objectAtIndex:0];
    ...
    if (retries > 0) {
        NSTimeInterval waitTime = [ADJUtil waitingTime:retries backoffStrategy:selfI.backoffStrategy];
        NSString * waitTimeFormatted = [ADJUtil secondsNumberFormat:waitTime];

        [selfI.logger verbose:@"Sleeping for %@ seconds before retrying sdk_click for the %d time", waitTimeFormatted, retries];
