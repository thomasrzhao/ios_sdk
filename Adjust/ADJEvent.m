...
static const NSUInteger kEventTokenLength = 6;
...
- (id)copyWithZone:(NSZone *)zone {
    ADJEvent *copy = [[[self class] allocWithZone:zone] initWithEventToken:[self.eventToken copyWithZone:zone]];
...
- (BOOL)checkEventToken:(NSString *)eventToken {
    ...
    if (eventToken.length != kEventTokenLength) {
...
- (void)addCallbackParameter:(NSString *)key value:(NSString *)value {
    if (![self isValidParameter:key attributeType:@"key" parameterName:@"Callback"]) {
        return;
    }

    if (![self isValidParameter:value attributeType:@"value" parameterName:@"Callback"]) {
        return;
    }
...
- (void)addPartnerParameter:(NSString *)key value:(NSString *)value {
    if (![self isValidParameter:key attributeType:@"key" parameterName:@"Partner"]) {
        return;
    }

    if (![self isValidParameter:value attributeType:@"value" parameterName:@"Partner"]) {
        return;
    }
